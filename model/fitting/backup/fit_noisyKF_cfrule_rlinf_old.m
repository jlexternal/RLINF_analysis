function [out] = fit_noisyKF_cfrule_rlinf(cfg)
...
vs = 0.0643; %0.0236 (old); % sampling variance 
    ...
    function [pt,mt,vt,ut,st,wt] = getp(alpha,delta,zeta,tau)
        ...
        for itrl = 1:ntrl
            ...
            if strcmpi(condstr,'fairy') & trl(itrl) == 2
                % first outcome seen, choice made (in experiment)
                if (c == 1 && rt_raw < 0) || (c == 2 && rt_raw >= 0)
                    et(itrl,c,:) = rt_raw-mt(itrl,c,:);
                    et(itrl,u,:) = -rt_raw-mt(itrl,u,:);
                else
                    et(itrl,c,:) = -rt_raw-mt(itrl,c,:);
                    et(itrl,u,:) = rt_raw-mt(itrl,u,:);
                end
            ...
            elseif strcmpi(condstr,'fairy') & trl(itrl) > 2
                % Fairy: outcome(t) - *learning(t)* - choice(t)
                if (c == 1 && rt_raw < 0) || (c == 2 && rt_raw >= 0)
                    et(itrl,c,:) = rt_raw-mt(itrl,c,:);
                else
                    et(itrl,c,:) = -rt_raw-mt(itrl,c,:);
            ...
            if cfrule % counterfactual rule
                ...
                elseif strcmpi(condstr,'fairy') & trl(itrl) > 2
                    if (u == 1 && rt_raw < 0) || (u == 2 && rt_raw >= 0)
                        et(itrl,u,:) = rt_raw-mt(itrl,c,:);
                    else
                        et(itrl,u,:) = -rt_raw-mt(itrl,c,:);
                    end
                end
                ...
                if strcmpi(condstr,'bandit')
                    ...
                    mt(itrl,u,:) = mt(itrl,u,:)+(delta*(-mt(itrl,u,:)));
                end
            end
            % account for drift
            vt(itrl,:) = vt(itrl,:)+vd;

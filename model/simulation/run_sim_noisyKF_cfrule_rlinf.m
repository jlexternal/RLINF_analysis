...
savename    = sprintf('out_sim_%s_nsim%d_%s',modelkernel,nsim,samplename);
%
%---------------------------------------------
...
for isubj = 1:nsubj
    fprintf('%d\n',isubj)
    if mod(isubj,10) == 0
        fprintf('Simulating KF model with counterfactual decay on subject %d...\n',isubj);
    end
    if isempty(out_vbmc{isubj,1})
        continue
    end
    ...
    for icond = 1:2 
        ...
        cfg.delta   = pars(isubj,2,icond);
        cfg.zeta    = pars(isubj,3,icond);
        cfg.tau     = pars(isubj,4,icond);
        cfg.nstype  = nstype;
        cfg.cfrule  = cfrule;
        ...
        cfg.trl     = trl';
        cfg.firstresp = resp1; % to match subject's first responses

        out_sim{isubj,icond} = sim_noisyKF_cfrule_rlinf(cfg);

        bmstate = bmst(dat.cond == icond-1);
        bmstate(bmstate == 0) = 2;
        out_sim{isubj,icond}.corr = repmat(bmstate,[nsim 1]) == out_sim{isubj,icond}.rt;
        fprintf('Accuracy cond %d: %.2f (alpha: %.2f, delta: %.2f\n',icond,mean(out_sim{isubj,icond}.corr,'all'),cfg.alpha,cfg.delta);
    end
end
